...
interface InpageNavigationProps {
  ...
  navigatePrevTab?: {
    active: boolean;
    onClick: (props: { toInitialTab?: boolean }) => void;
  };
...
const InpageNavigation: React.FC<InpageNavigationProps> = ({
  ...
  // 如果 routes[inPageNavIndex] 不一樣，
  // 要嘛就是同一個 routes index 的文字變了，比如 "Blogs Published" 變成 "Followers"
  // 要嘛就是 inpageNavIndex 變了，比如從 0 變成 1
  // 不管是哪種，只要有變化就點擊當前聚焦的 Tab，然後讓 hr 的長度及位置更新
  useEffect(() => {
    document.getElementById(routes[inPageNavIndex])?.click();
  }, [routes]);
  ...
  return (
    <>
      ...
      >
        {/* 回到前一個 tab */}
        <>
          {navigatePrevTab.active && (
            <button
              onClick={() => navigatePrevTab.onClick({})}
              className="
                max-md:hidden
                flex
                -mt-2
              ...
              "
            >
              <FlatIcons name="fi fi-sr-caret-left" className="text-xl" />
            </button>
          )}
        </>
